...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, scene: Scene, renderFunc,
                    renderArgs):
    if tb_writer:
...
class GUI:
    ...
    def train_step(self):
        ...
        render_pkg_re = render(viewpoint_cam, self.gaussians, self.pipe, self.background, kernel_size=0.0)
        ...
        with torch.no_grad():
            ...
            # Densification
            if self.iteration < self.opt.densify_until_iter and self.iteration < self.opt.iterations:
                ...
                if self.iteration > self.opt.densify_from_iter and self.iteration % self.opt.densification_interval == 0:
                    ...
                    self.gaussians.densify_and_prune(self.opt.densify_grad_threshold, 0.05, #0.005
        ...
        with torch.no_grad():
            ...
            if self.iteration == self.opt.iterations:
                ...
                training_report(self.tb_writer, self.iteration, Ll1, loss, l1_loss, self.iter_start.elapsed_time(self.iter_end), self.scene, ag_render, (self.pipe, self.background))
