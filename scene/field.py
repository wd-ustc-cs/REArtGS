...
class SDFNetwork(nn.Module):
    ...
    def positive_sdf(self, x):
        return self.forward(x)[:, :1].abs()
    ...
    # def gradient_for_mesh(self, x):
    #     with torch.set_grad_enabled(True):
    #         x.requires_grad_(True)
    #         y = self.positive_sdf(x)
    #         d_output = torch.ones_like(y, requires_grad=False, device=y.device)
    #         gradients = torch.autograd.grad(
    #             outputs=y,
    #             inputs=x,
    ...
    def gradient_for_mesh(self, x, chunk_size=1024*1024):
        r"""分批次计算sdf及其梯度，降低显存峰值占用。

        参数:
            x (torch.Tensor): 输入点云张量，形状通常为 `(batch_size, num_points, 3)`。
            chunk_size (int): 每批处理的点数量（需根据显存大小调整，如1024/2048）。

        返回:
            y (torch.Tensor): 所有点的sdf值，形状 `(batch_size, num_points, 1)`。
            gradients (torch.Tensor): 所有点的sdf梯度，形状 `(batch_size, num_points, 3)`。
        """
        device = x.device
        num_points, _ = x.shape  # 假设输入形状为 (batch_size, num_points, 3)
        num_chunks = (num_points + chunk_size - 1) // chunk_size  # 向上取整分块数

        y_list = []  # 存储各分块的sdf结果
        grad_list = []  # 存储各分块的梯度结果
        ...
        with torch.set_grad_enabled(True):
            for chunk_idx in range(num_chunks):
                ...
                x_chunk = x[start:end, :].requires_grad_(True)  # 仅当前分块开启梯度


                y_chunk = self.positive_sdf(x_chunk)
                d_output = torch.ones_like(y_chunk, requires_grad=False, device=device)
                ...
                grad_list.append(grad_chunk)
                torch.cuda.empty_cache()

        # 5. 合并所有分块的结果
        y = torch.cat(y_list, dim=0)
        gradients = torch.cat(grad_list, dim=1)
        return y, gradients.unsqueeze(1)
...
def fast_sdf2opacity(cos_anneal_ratio, xyz, sdf_network, deviation_network, viewpoint_cam, sample_dist, belta):
    sdf = sdf_network.sdf(xyz)

    sdf_grads = sdf_network.gradient(xyz).squeeze().clamp(-1e3, 1e3)
    inv_s = deviation_network(torch.zeros([1, 3]).cuda())[:, :1].clip(1e-6, 1e6).expand(
        sdf.shape)  # Single parameter
    # inv_s = inv_s.expand(batch_size * n_samples, 1)
    # gaussian_to_camera = torch.nn.functional.normalize(fov_cameras.get_camera_center() - self.points, dim=-1)
    ...
    dir_pp = (xyz - viewpoint_cam.camera_center.repeat(xyz.shape[0], 1))
    dir_pp_normalized = dir_pp / (dir_pp.norm(dim=1, keepdim=True) + 1e-8)
    true_cos = (dir_pp_normalized * sdf_grads).sum(-1, keepdim=True)
    # cos_anneal_ratio = 0.0
    iter_cos = -(F.relu(-true_cos * 0.5 + 0.5) * (1.0 - cos_anneal_ratio) +
                 F.relu(-true_cos) * cos_anneal_ratio)  # always non-positive
    ...
    scaled_prev = estimated_prev_sdf * inv_s
    scaled_next = estimated_next_sdf * inv_s
    prev_cdf = stable_sigmoid_derivative(scaled_prev, belta)
    next_cdf = stable_sigmoid_derivative(scaled_next, belta)
    ...
    #estimated_opacity = torch.sigmoid(estimated_opacity)
    #estimated_opacity = torch.sigmoid((p + 1e-5) / (c + 1e-5))
    return estimated_opacity.clamp(1e-5, 1.0), sdf, sdf_grads
