	...
	const bool debug)
{
  ...
  torch::Tensor t_out = torch::zeros({P, 1}, means3D.options().dtype(torch::kFloat32));
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
        ...
        t_out.contiguous().data<float>(),
        debug);
  }
  return std::make_tuple(rendered, out_color, radii, t_out, geomBuffer, binningBuffer, imgBuffer);
}
