...
from arguments import ModelParams, PipelineParams, onestage_OptimizationParams
...
from scene.field import  dist_sdf2opacity
...
def get_parameters_from_optimizer(optimizer):
    params = []
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    rendering = torch.clamp(
                        renderFunc(viewpoint, scene.gaussians, *renderArgs, use_sdf=scene.gaussians.use_sdf)["render"],
                        0.0, 1.0)
                    image = rendering[:3, :, :]
...
class GUI:
    def __init__(self, args, dataset, opt, pipe, testing_iterations, saving_iterations) -> None:
        ...
        self.gaussians = GaussianModel(dataset.sh_degree, use_sdf= dataset.use_sdf)



        self.scene = Scene(dataset, self.gaussians)

        self.gaussians.sdf_training_setup(opt)
        ...
        sphere_diameter = torch.tensor(self.trainCameras[0].zfar - self.trainCameras[0].znear).cuda()
        self.sample_dist = sphere_diameter / n_samples
        ...
        self.progress_bar = tqdm.tqdm(range(self.iteration, opt.iterations), desc="Training progress")
        # For UI
    ...
    def train_step(self):
        ...
        cos_anneal_ratio = self.iteration / (self.opt.iterations + 1)

        #self.gaussians._opacity, self.gaussians.sdf, sdf_gradients = dist_sdf2opacity(cos_anneal_ratio, self.gaussians, viewpoint_cam, self.sample_dist)
        self.gaussians._opacity, self.gaussians.sdf, sdf_gradients = dist_sdf2opacity(cos_anneal_ratio, self.gaussians, viewpoint_cam)

        sdf_gradient_loss = ((torch.linalg.norm(sdf_gradients, ord=2,
                                                dim=-1) - 1.0) ** 2).mean()


        render_pkg_re = render(viewpoint_cam, self.gaussians, self.pipe, self.background, self.dataset.kernel_size, use_sdf=self.dataset.use_sdf)

        # render
        rendering, viewspace_point_tensor, visibility_filter, radii, x_star= (render_pkg_re["render"], render_pkg_re["viewspace_points"],
                                                                        render_pkg_re["visibility_filter"], render_pkg_re["radii"], render_pkg_re["x_star"])


        if self.iteration > self.opt.densify_from_iter and self.iteration <self.opt.densify_until_iter:
            x_star_sdf = self.gaussians.sdf_network.sdf(x_star)
            #sdf_opacity_loss = (x_star_sdf ** 2).mean()
            sdf_opacity_loss = x_star_sdf.abs().mean()
        ...
        lambda_distortion = self.opt.lambda_distortion if self.iteration >= self.opt.distortion_from_iter else 0.0
        lambda_depth_normal = self.opt.lambda_depth_normal if self.iteration >= self.opt.depth_normal_from_iter else 0.0

        # Final loss
        loss = rgb_loss + depth_normal_loss * lambda_depth_normal + distortion_loss * lambda_distortion

        loss = loss + 0.1 * sdf_gradient_loss + 0.1 * sdf_normal_loss + 0.0001*sdf_opacity_loss
        if self.iteration % 1000 == 0:
            print(f"sdf_gradient_loss:{sdf_gradient_loss}, sdf_normal_loss:{sdf_normal_loss}, sdf_opacity_loss:{sdf_opacity_loss} ")
        loss = torch.nan_to_num(loss)
        #self.train_losses.append(rgb_loss.item())
        loss.backward()
        ...
        with torch.no_grad():
            ...
            if self.iteration in self.testing_iterations:
                ...
                cur_psnr = training_report(self.tb_writer, self.iteration, Ll1, loss, l1_loss,
                                           ...
                                           self.scene, render, (self.pipe, self.background, self.dataset.kernel_size),
            ...
            if self.iteration in self.saving_iterations:
                ...
                torch.save(sdf_gradients.detach(), os.path.join(self.dataset.model_path, f"{self.iteration}_sdf_grad.pth"))
                self.scene.save(self.iteration)
                self.gaussians.sdf_network.save_weights(self.dataset.model_path, self.iteration)
                torch.save({
                    "model_state_dict": self.gaussians.deviation_network.state_dict(),
                }, os.path.join(self.dataset.model_path, f'deviation_{self.iteration}.pth'))

                print("saving sdf_network")
            ...
            if self.iteration < self.opt.densify_until_iter:
                ...
                if self.iteration > self.opt.densify_from_iter and self.iteration % self.opt.densification_interval == 0:
                    ...
                    self.gaussians.densify_and_prune(self.opt.densify_grad_threshold, 0.05, self.scene.cameras_extent,   # 0.005
                                                     size_threshold)
                    print(f"gaussian_number:{self.gaussians.get_xyz.shape[0]}")
                    self.gaussians.compute_3D_filter(cameras=self.trainCameras)
                    print(f"gaussian_number:{self.gaussians.get_xyz.shape[0]}")



            if self.iteration % 100 == 0 and self.iteration > self.opt.densify_until_iter:
                if self.iteration < self.opt.iterations - 100:
            ...
            gs_params = get_parameters_from_optimizer(self.gaussians.optimizer)
            sdf_params = get_parameters_from_optimizer(self.gaussians.sdf_optimizer)
            torch.nn.utils.clip_grad_norm_(gs_params, max_norm=1)
            torch.nn.utils.clip_grad_norm_(sdf_params, max_norm=1)
            if self.iteration < self.opt.iterations:
                self.gaussians.optimizer.step()
                self.gaussians.update_learning_rate(self.iteration)
                self.gaussians.optimizer.zero_grad(set_to_none=True)
                self.gaussians.sdf_optimizer.step()
                self.gaussians.sdf_optimizer.zero_grad(set_to_none=True)

                self.gaussians.sdf_update_learning_rate(self.iteration)
    ...
    def test_step(self):
        ...
        self.gaussians._opacity, sdf, sdf_gradients = dist_sdf2opacity(cos_anneal_ratio, self.gaussians, cur_cam)
        del sdf, sdf_gradients
        out = ag_render(viewpoint_camera=cur_cam, pc=self.gaussians, pipe=self.pipe, bg_color=self.background)


        buffer_image = out[self.mode]  # [3, H, W]
        #buffer_image = out[self.mode][: 3,:,:]
    ...
    def train(self, iters=5000):
        self.gaussians.compute_3D_filter(cameras=self.trainCameras)
        if iters > 0:
            #tqdm.tqdm(range(self.iteration, opt.iterations), desc="Training progress")
            #for i in tqdm.trange(iters):
            for i in tqdm.trange(-1000, iters):
            #for i in tqdm.trange(0, iters):
                self.train_step()
...
if __name__ == "__main__":
    ...
    op = onestage_OptimizationParams(parser)
